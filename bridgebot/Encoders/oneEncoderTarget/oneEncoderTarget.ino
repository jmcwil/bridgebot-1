...
void count1() {
 ...
 void toTarget(int motorNum, int targetVal) {
  resetCounts();
  Serial.println("In function");
  while (counter[motorNum] < (targetVal - thresh)) {
    motor[motorNum].drive(speed[motorNum]);
  } 
  while (counter[motorNum] > (targetVal + thresh)) {
    motor[motorNum].drive(-speed[motorNum]);
  }
  motor[motorNum].drive(0);
  //Serial.print("Error: ");
  //Serial.print(counter[motorNum] - targetVal);
